...
public class Filereader {
    ...
    public int makeURMove() {
        int selectedColumn = UR.makeURMove(occupied, team);

        int row = occupied[selectedColumn];
        grid[row][selectedColumn] = team.charAt(0);
        occupied[selectedColumn]++;

        return selectedColumn;
    }
    public int makeMinMaxMove() {
        int depth = Integer.parseInt(param); 
        int selectedColumn = DLMM.makeMinMaxMove(grid, occupied, team, depth);
        // Update the board and occupied array with the selected move
        int row = occupied[selectedColumn - 1];
        grid[row][selectedColumn - 1] = team.charAt(0);
        occupied[selectedColumn - 1]++;
        return selectedColumn;
    }
}
